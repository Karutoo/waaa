...
public class AuthUserRepository implements IAuthUserRepository {
    ...
    @Override
    public int insertChat(String name, String time, String msg) {
        var sql = "insert into chat values (?, ?, ?)";
        var n = jdbc.update(sql, name, time, msg);
        return n;
    }
    ...
    public List<dwdawdw> findMsg(String ff) {
        // auth_user テーブルの user_name, user_pass を検索する
        String sql = "select user_name, MSG_BODY from chat WHERE USER_NAME LIKE '%'||?||'%'";
        ...
        List<dwdawdw> users = jdbc.query(sql,
                DataClassRowMapper.newInstance(dwdawdw.class), new Object[]{ff});
